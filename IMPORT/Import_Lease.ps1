...
function Insert_CommLeases_tmpLease {
    param (
        [string]$csvFilePath,
        [string]$databaseName,
        [string]$tableName
    ...
    $LeaseData = Get-Content $csvFilePath | Select-Object -Skip 1 | ForEach-Object { $_  -replace "'"," "    -replace '\s+$', '' }
...
Write-Output "$line"
       ...
       (    '$($columns[0])' --[Property_code]
           ,'' --[Floor_code]
           ,'' --[Level]
           ,'$($columns[1])' --[Lease_code]
           ,'$($columns[2])' --[Lease_Name]
           ,'$($columns[3])' --[Customer_Code]
           ,'$($columns[4])' --[Company_Name]
           ,'' --[TenantName]
           ,'' --[ExternalTenantCode]
           ,'' --[buildingid]
           ,'' --[propertyid]
...
invoke-sqlcmd -Server "MRIPF3DNGQF" -database "v3_common" -Query "delete from tmpLease_JBGSmith"

Insert_CommLeases_tmpLease -csvFilePath "C:\CopilotDCA\Repo\IMPORT\ImportFiles\CommLeases*.csv" -databaseName "v3_common" -tableName "tmpLease_JBGSmith"
########################################

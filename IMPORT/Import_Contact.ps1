...
function Insert_CommContacts_tmpContact {
    param (
        [string]$csvFilePath,
        [string]$databaseName,
        [string]$tableName
    ...
    if (-not (Test-Path $csvFilePath)) {
        ...
        exit 1  # Stop the script if the CSV file is missing
    }
    ...
    $contactData = Get-Content $csvFilePath | Select-Object -Skip 1 | ForEach-Object { $_ -replace "'"," "    -replace '\s+$', '' }
...
"@
    ...
    foreach ($lines in $contactData) {
        $line = Clean_CsvLine $lines
        $columns = $line -split "," 
Write-Output "$line"
        $sqlInsertValues = @"
           ('$($columns[0])' --[Record_code]
           ,'$($columns[1])' --[Role_Description]
           ,'$($columns[2])' --[Last_name]
           ,'$($columns[3])' --[Contact_Code]
           ,'$($columns[4])' --[First_Name]
           ,'$($columns[5])' --[Title]
           ,'$($columns[6])' --[Address_1]
           ,'$($columns[7])' --[Address_2]
           ,'$($columns[8])' --[Address_3]
           ...
           ,'$($columns[12])' --[Zip_Code]
           ,'$($columns[13])' --[Country]
           ,'$($columns[14])' --[PhoneNumber_1]
           ,'$($columns[15])' --[Email]
           ,'$($columns[16])' --[Alternate_Email]
           ,'' --[externalLeaseCode]
           ,'' --[ContactLastName]
           ,'' --[ContactExternalCode]
           ,'' --[ContactFirstName]
...
invoke-sqlcmd -Server "MRIPF3DNGQF" -database "v3_common" -Query "delete from tmpContact_JBGSmith"

Insert_CommContacts_tmpContact  -csvFilePath "C:\CopilotDCA\Repo\IMPORT\ImportFiles\CommContacts*.csv" -databaseName "v3_common" -tableName "tmpContact_JBGSmith"
########################################
